...
class BotCommonCommands:
    ...
    async def meme(self, ctx, *args):
        ...
        if 3 >= len(args) > 0 == error_count:
            ...
            result = r.json()
            if result['success']:
                await self.bot.send_message(ctx.message.channel, str(result['data']['url']) + " ID Meme:" + str(generator_id))
            else:
                print("Error:" + str(result['error_message']))
        else:
            await self.bot.send_message(ctx.message.channel, "**Usage:** !meme \"text1\" \"text2 or memeId\"[Optional] \"memeId\"[Optional]")
        print("-------------------------")
    ...
    async def printtext(self, ctx, *args):
        ...
        if len(args) == 1:
            ...
            await self.bot.send_message(ctx.message.channel, "**Usage:** !printtext \"phrase\"")
    ...
    async def weather(self, ctx, *args):
        ...
        if len(args) < 1 or len(args) > 3:
            await self.bot.send_message(ctx.message.channel, "**Please read:** " + self.botVariables.get_command_prefix() + "help weather")
            return
        else:
            country_code = False
                ...
                if len(args) == 2:  # if 2 params
                    if not country_code:  # country code not present
                        ...
                        try:
                            ...
                            await self.bot.send_message(ctx.message.channel, "**Check the day or the country code, please read:** " + self.botVariables.get_command_prefix() + "help weather")
                            return
                    else:
                        city_name = str(args[1])
                if len(args) == 3:
                    if not country_code:
                        await self.bot.send_message(ctx.message.channel, "**Check the country code, please read:** " + self.botVariables.get_command_prefix() + "help weather")
                        return
                    else:
                        city_name = str(args[1])
                        try:
                            ...
                            return
            if day > 19:
                await self.bot.send_message(ctx.message.channel, "**Check the day code, please read:** " + self.botVariables.get_command_prefix() + "help weather")
                return
            ...
            try:
                await self.bot.send_message(ctx.message.channel, "**Error:** "+str(request_result["response"]["error"]["description"]) + "(check usage with " + self.botVariables.get_command_prefix() + "help weather)")
                ...
                try:
                    ...
                    await self.bot.send_message(ctx.message.channel, "*A strange error occurred, cannot retrieve that city, sorry...*")
            print("-------------------------")
    ...
    async def hacked(self, ctx, *args):
        ...
        if len(args) == 1:
            ...
            await self.bot.send_message(ctx.message.channel, "**Usage:** !hacked \"email or username\"")
        print("-------------------------")
    ...
    async def ur(self, ctx, *args):
        ...
        if len(args) == 1:
            ...
            if request_result["result_type"] == "no_results":  # no results found
                ...
                await self.bot.send_message(ctx.message.channel, "*No results found for \"" + str(args[0]) + "\" in urban dictionary*")
            else:
                ...
                if request_result["result_type"] == "exact":  # found an exact result
                    for definition in request_result["list"]:  # choose the best result with positive-percentage
                        percentage = (definition["thumbs_up"] / (definition["thumbs_up"] + definition["thumbs_down"])) * 100
                        if percentage > current_max_vote:
                            saved_definition = definition
                            current_max_vote = percentage
                    # create the embed to send
                    embed = discord.Embed(title="Urban Dictionary - Link", url=saved_definition["permalink"], color=0x1d2439,
                                          description="Best dictionary result for \""+str(args[0])+"\"")
                    embed.set_author(name="Search required by " + ctx.message.author.name,
                                     icon_url=ctx.message.author.avatar_url)
                    embed.set_thumbnail(url='https://cdn.discordapp.com/attachments/276674976210485248/350641481872179200/featured-image4.jpg')
            ...
            await self.bot.send_message(ctx.message.channel, "**Usage:** !ur word")
        print("-------------------------")
    # ---------------------------------------------------------------------
