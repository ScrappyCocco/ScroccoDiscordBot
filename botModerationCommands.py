    ...
    async def banuser(self, ctx):
        ...
        if BotMethods.is_server_admin(ctx.message.author):
            ...
            try:
                ...
                await self.bot.say(str(ctx.message.mentions[0].name) + " has been banned")
    ...
    async def unbanuser(self, ctx):
        ...
        if BotMethods.is_server_admin(ctx.message.author):
            ...
            try:
                ...
                await self.bot.say(str(ctx.message.mentions[0].name) + " has been un-banned")
            except discord.errors.Forbidden:
                await self.bot.say("Sorry, I don't have the `ban` permission")
            print("-------------------------")
        else:
    ...
    async def softban(self, ctx):
        ...
        if BotMethods.is_server_admin(ctx.message.author):
            ...
            try:
                await self.bot.ban(ctx.message.mentions[0], delete_message_days=100)
                await self.bot.unban(ctx.message.server, ctx.message.mentions[0])
                await self.bot.say(str(ctx.message.mentions[0].name) + " has been soft-banned ")
            except discord.errors.Forbidden:
                await self.bot.say("Sorry, I don't have the `softban` permission")
            print("-------------------------")
        else:
            await self.bot.say("You don't have access to this command  :stuck_out_tongue: ")
    ...
    async def kickuser(self, ctx):
        ...
        if BotMethods.is_server_admin(ctx.message.author):
            ...
            try:
                ...
                await self.bot.say(str(ctx.message.mentions[0].name) + " has been kicked")
            except discord.errors.Forbidden:
